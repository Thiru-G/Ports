...
volatile bool irSensorState = LOW;       // Current state of the IR sensor
String qrCodeData = "";
...
void setup() {
  ...
  // Setup interrupts for IR sensor to detect both edges
  attachInterrupt(digitalPinToInterrupt(irSensorPin), irSensorISR, CHANGE);  // IR sensor interrupt (detect both HIGH and LOW)
}
...
void loop() {
  ...
  if (irSensorDetected) {
    if (irSensorState == HIGH) {
      Serial.println("IR_SENSOR::: Sniff started");  // IR sensor went HIGH
    } else {
      Serial.println("IR_SENSOR::: Sniff stopped");  // IR sensor went LOW
    }
    irSensorDetected = false;  // Reset flag after handling
  }
...
void IRAM_ATTR irSensorISR() {
  ...
  irSensorDetected = true;  // Set flag when IR sensor state changes
}
